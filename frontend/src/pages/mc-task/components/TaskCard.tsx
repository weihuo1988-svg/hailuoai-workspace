...
interface TaskCardProps {
  ...
  onGuideComplete?: (id: string) => void;
}

export function TaskCard({ task, onComplete, onDelete, showPasswordOnComplete, onGuideComplete }: TaskCardProps) {
  const [open, setOpen] = useState(false);
  const [pw, setPw]     = useState('');
  const [err, setErr]   = useState(false);
  const ref = useRef<HTMLInputElement>(null);
  const isGuideTask = task.id.startsWith('guide_');
  ...
  return (
    <>
      <div style={{
        background: isGuideTask ? 'rgba(30,50,30,0.95)' : 'rgba(30,30,50,0.88)',
        border: `3px solid ${isGuideTask ? '#FFD700' : '#5C9E3E'}`,
        borderRadius: 0, padding: 14, marginBottom: 10, position: 'relative',
        boxShadow: isGuideTask ? '4px 4px 0 #B8860B, 0 0 20px rgba(255,215,0,0.3)' : '4px 4px 0 #1B5E20',
        backdropFilter: 'blur(4px)',
        animation: isGuideTask ? 'fadeIn 0.2s ease, pulse 1.5s ease-in-out infinite' : 'fadeIn 0.2s ease',
      }}>
        {isGuideTask && (
          <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
            ...
            <button onClick={handleClick} style={{
              background: isGuideTask ? 'linear-gradient(180deg,#FFD700,#B8860B)' : 'linear-gradient(180deg,#4CAF50,#2E7D32)',
              border: `3px solid ${isGuideTask ? '#B8860B' : '#1B5E20'}`,
              color: isGuideTask ? '#1a1a2e' : '#fff',
              fontFamily: "'Press Start 2P',monospace", fontSize: 7, padding: '8px 12px',
              cursor: 'pointer', borderRadius: 0,
              boxShadow: isGuideTask ? '3px 3px 0 #B8860B' : '3px 3px 0 #1B5E20',
              animation: isGuideTask ? 'pulse 1s ease-in-out infinite' : 'none',
            }}>
              {isGuideTask ? '免密完成' : '完成了！'}
            </button>
          </div>
        </div>
      </div>
