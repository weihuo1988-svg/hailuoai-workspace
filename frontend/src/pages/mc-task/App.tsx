...
import type { AppState, Tab, GuideStep } from './types';
...
export default function McTaskApp() {
  ...
  const [guideStep, setGuideStep]       = useState<GuideStep>('none');
  const [showDeviceCodeModal, setShowDeviceCodeModal] = useState(false);

  const isNewUser = state.isNewUser && !state.guideDeviceCode;

  useEffect(() => {
    if (isNewUser && guideStep === 'none') {
      setGuideStep('guide-task');
    }
  }, [isNewUser, guideStep]);

  useEffect(() => {
    if (tab === 'collection' && guideStep === 'guide-shop') {
      setTimeout(() => {
        setGuideStep('device-code');
  ...
  const handleGuideComplete = useCallback((taskId: string) => {
    if (taskId.startsWith('guide_') && guideStep === 'guide-task') {
      setGuideStep('guide-chest');
    }
  }, [guideStep]);

  const handleDeviceCodeConfirm = useCallback((code: string) => {
    const bonusChests = Array.from({ length: 3 }, () => ({ id: uid(), createdAt: new Date().toISOString() }));
    setState((s) => ({
      ...s,
      guideDeviceCode: code,
      chests: [...s.chests, ...bonusChests],
      isNewUser: false,
    }));
    setShowDeviceCodeModal(false);
    setGuideStep('none');
    setNotif('🎁 恭喜！获得 3 个高级宝箱奖励！');
  }, []);
  ...
  const openChest = useCallback((chestId: string) => {
    ...
    if (guideStep === 'guide-chest') {
      setTimeout(() => setGuideStep('guide-shop'), 1000);
    }
  }, [guideStep]);
  ...
  return (
    <div style={{ minHeight: '100dvh', position: 'relative', overflowX: 'hidden', background: '#0a0a1e' }}>
      ...
      <GuideOverlay
        step={guideStep}
        onNext={() => setGuideStep('none')}
        onClose={() => {}}
      />
      
      {showDeviceCodeModal && (
        <DeviceCodeModal onConfirm={handleDeviceCodeConfirm} />
      )}
    </div>
  );
}
