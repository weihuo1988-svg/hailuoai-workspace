...
/** DashScope CosyVoice TTS — 浏览器直连 WebSocket（无需后端） */
async function dashscopeTTS(text: string, voiceType: VoiceType): Promise<void> {
  ...
  const API_KEY = 'sk-973671b9760f45c5b74322a8ae087279'
  const taskId = crypto.randomUUID().replace(/-/g, '')
  const audioChunks: Uint8Array[] = []

  return new Promise((resolve, reject) => {
    const ws = new WebSocket(
      `wss://dashscope.aliyuncs.com/api-ws/v1/inference?Authorization=Bearer%20${API_KEY}&task_id=${taskId}`
    )

    let textSent = false

    ws.addEventListener('open', () => {
      // Step 1: run-task（不放文本，input 为空）
      ws.send(JSON.stringify({
        ...
        payload: {
          ...
          parameters: {
            ...
            sample_rate: 22050, rate: 1, pitch: 1,
          },
        },
      }))
    })
    ...
    ws.addEventListener('message', (e) => {
      if (e.data instanceof ArrayBuffer || e.data instanceof Buffer) {
        const buf = e.data instanceof Buffer ? new Uint8Array(e.data) : new Uint8Array(e.data)
        if (buf.length > 10) audioChunks.push(buf)
        return
      }
      try {
        ...
        if (event === 'task-failed') {
          reject(new Error(data.header?.error_message || 'TTS failed'))
          ws.close()
        }
      } catch {}
    })
    ...
    ws.addEventListener('close', () => {
      ...
      // 合并音频块
      const total = audioChunks.reduce((a, b) => a + b.length, 0)
